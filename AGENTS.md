...
Vuora は Chrome / Firefox 拡張機能 (Manifest V3)。Web ブラウジングを快適にする 12 機能を提供する。
...
| `globalThis` 公開定数 | 28 | `actions.js` の 24 名前空間（`test/actions.test.js` の `required` 配列が単一情報源）+ ScanRunner + AudioPipeline + CleanerCore + SettingsBackup。※ `rg "globalThis\.\w+ = "` は二重ロード防止フラグ（`__cpaActionsLoaded` / `__cpaAudioPipelineLoaded`）も拾うので、素の grep 件数は名前空間数と一致しない |
| カラーピッカー履歴上限 | 20 件 | `ColorPicker.HISTORY_LIMIT` |
| popup タブ数 | 7 | `PopupTabs.ALL`（調整 / YouTube / X / Instagram / TikTok / カラーピッカー / 設定） |
| YouTube 機能拡張 カテゴリ数 | 5 | `SearchFixer.CATEGORIES`（menu_ui / video_filter / watch_page / search_only / integration） |

### 機能一覧

1. **YouTube 機能拡張** — Shorts 削除・コメント欄非表示・ライブチャット非表示・登録チャンネル拡張・接続モニター・配信時刻オーバーレイ（配信アーカイブに配信時刻を重ねる）・海外チャンネル除外・Gemini Notebook 送信を含む 35 サブ機能
...
| `test/settings-backup.test.js` | 設定JSONの往復・許可リスト・型と容量の検証 |
...
設定キーや保存経路を変更するときは、DESIGN.md の[任意のPC間同期](DESIGN.md#任意のpc間同期)と[手動の設定バックアップ](DESIGN.md#手動の設定バックアップ)を読み、`src/background/settings-sync.js` の同期対象・競合解決単位と `src/lib/settings-backup.js` の許可リスト・既定値・検証規則を確認する。共有スクリプトの追加時は Chrome の `importScripts` と Firefox manifest の `background.scripts` の読み込み順を照合する。検証は `test/settings-sync.test.js`、`test/settings-application.test.js`、`test/settings-backup.test.js` を含む `pnpm test` で行う。
