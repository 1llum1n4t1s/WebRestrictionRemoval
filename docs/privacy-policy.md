...
本拡張機能は、以下の設定データをユーザーの端末内（`chrome.storage.local`）に保存します。設定同期を有効にした場合の共有範囲は「例外 5」を参照してください。

- **`searchFixerEnabled`**（真偽値）: YouTube 機能拡張（Shorts 削除・コメント欄非表示・ライブチャット非表示・登録チャンネル拡張・接続モニターを含む 35 サブ機能の親）の有効/無効。
- **`searchFixerFeatures`**（オブジェクト）: YouTube 機能拡張の 35 個のサブ機能（Shorts 削除 / 検索結果ノイズ除去 / 動画属性削除 / ハイライト / 動画ページ整形〈コメント欄非表示・ライブチャット非表示〉/ レイアウト / 登録チャンネル拡張 / 接続モニター）の個別 ON/OFF 状態。
- **`searchFixerGridItems`**（数値）: YouTube ホームグリッドの列数指定（0=自動 / 4 / 5 / 6）。
- **`searchFixerBlockedChannels`**（配列）: チャンネルブロックリスト機能で登録したチャンネルの識別子（ハンドルまたはチャンネル ID）と表示名のリスト。登録は検索結果のチャンネル名横に表示されるボタンから行い、登録済みチャンネルの動画は検索結果に加えホーム・登録チャンネル・急上昇等の YouTube フィードページ全体から除去されます。通常は端末内に保存されます。設定同期を有効にすると、このリストも同期します（例外 5）。
...
- **`popupLastTab`**（文字列・`"tune"` / `"youtube"` / `"x"` / `"instagram"` / `"tiktok"` / `"picker"` / `"settings"` のいずれか）: ポップアップで最後に開いていたタブ。次回起動時の表示状態復元に使用。旧値 `"assist"` は `"tune"` に自動変換されます。

これらの値は通常、端末内に保存されます。設定同期を有効にした場合は、例外 5 に記載する設定だけをブラウザの同期サービスへ保存します。
...
「07 設定」タブで利用者が操作したときだけ、機能設定・EQ・除外チャンネル・採色履歴・表示タブをJSONファイルとして端末へ保存し、利用者が選んだファイルから読み込めます。認証情報・送信先アカウント・キャッシュ・同期のON/OFF・サイト上の表示位置は含めません。ファイルを開発者や外部サーバーへ送信することはありません。設定同期がONの場合、読み込んだ設定のうち例外5の対象項目はブラウザの同期サービスへ保存されます。
...
設定同期を有効にした場合は、例外 5 に記載する設定をブラウザ提供者の同期サービスと共有します。広告・分析目的でデータを共有することはありません。お問い合わせフォームから送信された内容は、返信のために開発者（Kagayoi）のサポート窓口でのみ取り扱います。

## ネットワーク通信

本拡張機能は、以下に明示する 5 つの例外を除き、外部サーバーへの通信を一切行いません。
...
「複数PC間で設定を同期」は初期状態 OFF です。各PCで有効にすると、機能の ON/OFF、サブ機能、音量・イコライザ、映像・ルーペ、チャンネル除外リスト、色のコピー形式をブラウザ標準の storage.sync に保存します。ブラウザ側で同期を有効にしている場合、ブラウザ提供者（Chrome では Google、Firefox では Mozilla）の同期サービスを通じて、同じブラウザ・同じアカウントのPCに共有されます。開発者のサーバーには送信しません。認証情報、送信先アカウント、アカウント一覧キャッシュ、採色履歴、ポップアップの表示タブ、HUD の位置、閲覧 URL・ページ内容・音声・画像は同期対象外です。競合解決のため、設定変更時刻・論理カウンター・拡張機能が生成するランダムな端末ID・削除記録も同期します。端末IDは機器名やアカウント情報から作らず、広告や分析には使用しません。同期の ON/OFF と適用状態は端末内に保存します。OFF にするとこのPCからの設定の送受信を止めますが、ブラウザに保存済みの同期データは削除しません。

## 権限の使用目的

- **activeTab**: ポップアップで設定を変更した際、現在のタブ情報（音量ブースターの対象タブ判定など）にアクセスするために使用します。
- **storage**: 上記「ローカルに保存するデータ」の各キーを端末内に保存・復元し、設定同期を有効にした場合は対象設定をブラウザの同期ストレージへ保存するために使用します。
