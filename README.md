...
外部（第三者）サーバーへの通信は発生せず、アクセス中のサイト自身への同一オリジン GET のみです。

**制限事項**: 合成 `mousemove` は **クライアント側 JS のアイドル検知** のみリセットします。サーバー側のセッション Cookie / JWT / 認証プロキシ（Zscaler 等）のタイムアウトは HTTP ping が無いサイト（Box や多くの汎用 SaaS）では延長されません。これらのサイトでは本機能を有効にしていてもサーバー側セッション切れによる再ログインが発生することがあります。
