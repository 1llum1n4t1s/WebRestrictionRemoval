...
- **全 fetch に `AbortSignal.timeout(NotebookLm.FETCH_TIMEOUT_MS)` 必須**: 無いと送信ボタンが「送信中…」で固着して再送不能になる（/rere RC-C）。`redirect: "manual"` も CLAUDE.md §外部 fetch allowlist 設計の共通要件どおり適用する（未ログイン時は opaqueredirect → `res.ok === false` → `not-authorized` に落ちるので認証検出は成立する / rere RC-F）
